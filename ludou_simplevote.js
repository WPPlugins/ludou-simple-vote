...
function ludou_simple_vote(button, wpurl, postid, fen) {
	...
	var url = wpurl + "/wp-content/plugins/ludou-simple-vote/postratings.php";
	var parameters = "id=" + postid + '&fen=' + fen;

	ludou_xmlHttp.onreadystatechange = ludou_simple_vote_change;
	ludou_xmlHttp.open("POST", url, true);
	ludou_xmlHttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	ludou_xmlHttp.setRequestHeader("Content-length", parameters.length);
	ludou_xmlHttp.setRequestHeader("Connection", "close");
	ludou_xmlHttp.setRequestHeader("If-Modified-Since", "0");
	ludou_xmlHttp.send(parameters);
}
